        ...
        <?php if(isset($job[0]['items']) && count($job[0]['items']) > 0): ?>
            ...
            <div class="device-info">            
                <div class="row">
                    ...
                    <div class="col-md-2">
                        <div class="form-group">
                            <label for="">Warranty Yes/No</label>
...
$(document).ready(function(){
    $('.brand').on('change', function (){
        var brand = $(this);
        $.ajax({
            url: '<?php echo site_url('job/ajax_get_model') ?>',
            data: { brand_id: brand.val() },
            dataType: 'json',
            success: function(data){
                var options = '<option></option>';
                var modelSelect = brand.closest('.row').find('.model');
                for (var x = 0; x < data.length; x++) {
                    options += '<option value="' + data[x]['id'] + '">' + data[x]['title'] + '</option>';
                }
                modelSelect.html(options);
                // if there's a hidden input with existing value, set it immediately
                var selected = modelSelect.siblings('input[type=hidden]').val();
                if (typeof selected !== 'undefined' && selected !== '') {
                    modelSelect.val(selected);
                }
            },
            error: function(jqXHR, textStatus, errorThrown){
                console.error('Failed to load models for brand ' + brand.val() + ': ' + textStatus);
            }
        });
    });
    <?php if(isset($job[0]['items']) && count($job[0]['items']) > 0): ?>
...
function save()
{
    ...
    $.ajax({
        ...
        error: function (jqXHR, textStatus, errorThrown)
        {
            ...
            var msg = jqXHR.responseText ? jqXHR.responseText.substring(0,200) : textStatus;
            alert('Error adding / update data: ' + msg);
            $('#btnSave').text('save'); //change button text
            $('#btnSave').attr('disabled',false); //set button enable
