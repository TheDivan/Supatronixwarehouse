...
{	
	public function __construct() 
	{
		...
		$this->CurrencyCode = array(
			'USD' => '$', 
			// Use Namibian Dollar symbol for local currency
			'GBP' => 'N$', 
			'NAD' => 'N$', 
			'EUR' => '&euro;' 
		); 
	...
	public function sent_email($from_email, $from_name, $to_email, $subject, $message, $attachments = array() )
	{
		...
		// Quick SMTP defaults using Gmail - replace with real credentials or set in config
		$config['protocol'] = 'smtp';
		$config['smtp_host'] = 'smtp.gmail.com';
		$config['smtp_user'] = $this->CI->config->item('smtp_user') ?: 'your_gmail_username@gmail.com';
		$config['smtp_pass'] = $this->CI->config->item('smtp_pass') ?: 'your_app_password';
		$config['smtp_port'] = $this->CI->config->item('smtp_port') ?: 587;
		$config['smtp_crypto'] = $this->CI->config->item('smtp_crypto') ?: 'tls';
		...
		// Wrap send in try/catch so email failures don't break the main flow
		try {
			$sent = $this->CI->email->send();
			if ($sent === FALSE) {
				$debug = $this->CI->email->print_debugger(array('headers'));
				$log = sprintf("[%s] Email send returned false. Debug: %s\n", date('Y-m-d H:i:s'), $debug);
				@file_put_contents(APPPATH.'logs/email_errors.log', $log, FILE_APPEND);
				log_message('error', $log);
			}
			return $sent;
		} catch (Exception $e) {
			$log = sprintf("[%s] Exception while sending email: %s in %s:%d\nStack: %s\n", date('Y-m-d H:i:s'), $e->getMessage(), $e->getFile(), $e->getLine(), $e->getTraceAsString());
			@file_put_contents(APPPATH.'logs/email_errors.log', $log, FILE_APPEND);
			log_message('error', $log);
			// Do not rethrow; return false so calling code can continue
			return false;
		}
	}
