...
class Job_model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        $this->column = array(
            ...
            "$this->table_customer.phone",
            "$this->table_job_items.device_number",
    ...
    private function _get_datatables_query()
    {
        ...
        // select job + customer fields and join job_items (to allow searching IMEI/device numbers)
        $this->db->select("{$this->table}.*")
            ->select("{$this->table_customer}.name, {$this->table_customer}.phone")
            ->from($this->table_customer)
            ->join($this->table, "{$this->table}.customer_id = {$this->table_customer}.id", 'inner')
            ->join($this->table_job_items, "{$this->table_job_items}.job_id = {$this->table}.id", 'left');
        ...
        // group by job id to avoid duplicate rows when joining job_items
    ...
    {
        $this->db->select("{$this->table_customer}.name, {$this->table_customer}.email, {$this->table}.*")
        ->from($this->table_customer)
        ->join($this->table, "{$this->table}.customer_id = {$this->table_customer}.id", 'inner');
    ...
    /**
     * Search jobs by customer name, phone, device IMEI/serial, or dates.
     * Returns up to $limit matching jobs.
     */
    public function search_jobs($q, $limit = 50)
    {
        if (empty($q)) return array();

        $this->db->select("{$this->table}.id, {$this->table}.receive_date, {$this->table}.delivery_date, {$this->table}.status, {$this->table}.updated_datetime")
            ->select("{$this->table_customer}.name, {$this->table_customer}.phone, {$this->table_customer}.email")
            ->select("{$this->table_job_items}.device_number")
            ->from($this->table)
            ->join($this->table_customer, "{$this->table}.customer_id = {$this->table_customer}.id", 'inner')
            ->join($this->table_job_items, "{$this->table_job_items}.job_id = {$this->table}.id", 'left');
        ...
        $this->db->group_start();
        $this->db->like("{$this->table_customer}.name", $q);
        $this->db->or_like("{$this->table_customer}.phone", $q);
        $this->db->or_like("{$this->table_job_items}.device_number", $q);
        // allow searching dates as strings (YYYY-MM-DD or partial)
        $this->db->or_like("{$this->table}.receive_date", $q);
        $this->db->or_like("{$this->table}.delivery_date", $q);
        $this->db->group_end();
